...
public class UrlAliasService {
    ...
    public String getOriginalUrl(String requestedAlias) {
        ...
        Optional<BDUrlAlias> alias = dao.getOriginalByRequestAlias(requestedAlias);
        if(alias.isPresent()){
            return alias.get().getOriginalUrl();
        } else {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "No such resource exists");
        }
    }
}
